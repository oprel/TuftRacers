...
public class CustomIK : MonoBehaviour {
	...
	void CalculateIK(){
		...
		if (targetDistance < armLength && targetDistance > minimumLength) {
            if (angleUpperArm>0)	rootBone.localEulerAngles = new Vector3(- angleUpperArm, 0, 0);
            if (angleElbow>0)		elbowBone.localEulerAngles = new Vector3(180f - angleElbow, 0, 0);
			//rootBone.rotation = Quaternion.Euler(new Vector3(- angleUpperArm, 0, 0));
            //elbowBone.rotation = Quaternion.Euler(new Vector3(180f - angleElbow, 0, 0));
