...
namespace AwesomeTechnologies.Vegetation.PersistentStorage
{
    ...
    public class PersistentVegetationStorageEditor : VegetationStudioBaseEditor
    {
        ...
        private void DrawStoredVegetationInspector()
        {
            ...
            {
                GUILayout.BeginVertical("box");
                EditorGUILayout.HelpBox("There is no Vegetation Items stored in this storage. To add bake vegetation from the rules, paint with the tool or import from 3rd party systems.", MessageType.Info);
                GUILayout.EndVertical();
        ...
        private void DrawImportInspector()
        {
            ...
            for (int i = 0; i <= _persistentVegetationStorage.VegetationImporterList.Count - 1; i++)
            {
                ...
                _persistentVegetationStorage.VegetationImporterList[i].VegetationPackage =
                    _persistentVegetationStorage.VegetationSystem.CurrentVegetationPackage;
        ...
        void PrecisionPaintItem(bool addVegetationItem)
        {
            ...
            if (_sceneMeshRaycaster.RaycastSceneMeshes(ray, out raycastHit, true, includeColliders, includeMeshes))
            {
                ...
                {
                    EraseVegetationItem(raycastHit.point, _persistentVegetationStorage.SelectedPrecisionPaintingVegetationID, _persistentVegetationStorage.SampleDistance);
                }
                else
                {
        ...
        private void PaintVegetationItems(Vector3 hitPosition, bool addVegetationItems)
        {
            ...
            for (int x = 0; x <= xCount - 1; x++)
            {
                for (int z = 0; z <= zCount - 1; z++)
                {
                    ...
                    if (Event.current.control)
                    {
                        EraseVegetationItem(randomizedPosition, _persistentVegetationStorage.SelectedPaintVegetationID, currentSampleDistance);
                    }
                    else
                    {
        ...
        void AddVegetationItem(Vector3 worldPosition, Vector3 terrainNormal, string vegetationItemID, float sampleDistance)
        {
            ...
            VegetationItemInfo vegetationItemInfo = _persistentVegetationStorage.VegetationSystem
                .CurrentVegetationPackage.GetVegetationInfo(vegetationItemID);
