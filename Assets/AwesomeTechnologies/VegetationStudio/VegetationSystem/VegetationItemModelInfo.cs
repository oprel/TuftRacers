...
namespace AwesomeTechnologies
{
    ...
    public class VegetationItemModelInfo
    {
        ...
        public void AddVegetationModel(GameObject rootVegetationModel, int lodIndex)
        {
            RegisterWindIDs();
            ...
            LOD1Distance = GetLODDistance(rootVegetationModel, 0);
            LOD2Distance = GetLODDistance(rootVegetationModel, 1);
        ...
        Mesh GetVegetationMesh(GameObject rootVegetationModel,int lodIndex)
        {
        ...
        private Material[] CreateMaterials(Material[] sharedMaterials, int lod)
        {
            ...
            for (int i = 0; i <= sharedMaterials.Length - 1; i++)
            {
                ...
                RefreshMaterial(materials[i],lod);
            }
            return materials;
        }
        ...
        {           
            ...
            {
                VegetationMaterialsLOD0[i].SetTexture("_MainTex", texture);
            }
        ...
        public void RefreshMaterial(Material material, int lod)
        {
            if (VegetationRenderType == VegetationRenderType.InstancedIndirect)
            {
                if (!VegetationSystem.UseGPUCulling || !VegetationSystem.UseComputeShaders || !SystemInfo.supportsComputeShaders)
                {
            ...
            material.SetFloat("_CullFarStart", VegetationSettings.VegetationDistance);
