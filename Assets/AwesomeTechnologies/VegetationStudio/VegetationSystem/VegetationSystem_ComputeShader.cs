...
namespace AwesomeTechnologies
{
    ...
    public partial class VegetationSystem
    {
        ...
        {
            if (_dummyComputeBuffer != null) _dummyComputeBuffer.Dispose();
        }

        void SetupFrustumComputeShaders()
        {
            DummyMaskTexture = new Texture2D(2, 2);
            _dummyComputeBuffer = new ComputeBuffer(1, (16 * 4 * 2) + 16, ComputeBufferType.Default);
        ...
        void DrawCellsIndirectComputeShader()
        {
            ...
            for (int i = 0; i <= VegetationModelInfoList.Count - 1; i++)
            {
                ...
                for (int j = 0; j <= _hasBufferList.Count - 1; j += buffercount)  
                {
                    ...
                    for (int k = 0; k <= VegetationModelInfoList[i].VegetationMeshLod0.subMeshCount - 1; k++)
                    {
                ...
                float lod1Distance = VegetationModelInfoList[i].LOD1Distance * QualitySettings.lodBias * CurrentVegetationPackage.VegetationInfoList[i].LODFactor;
                float lod2Distance = VegetationModelInfoList[i].LOD2Distance * QualitySettings.lodBias * CurrentVegetationPackage.VegetationInfoList[i].LODFactor;
                ...
                if (UseGPUCulling)
                {
                    ...
                    FrusumMatrixShader.SetBool(_useLodsID, UseIndirectLoDs && !vegetationItemInfo.DisableLOD);
        ...
        void DrawMergedVegetationCellBuffer(int i, ComputeBuffer visibleBuffer, List<ComputeBuffer> argBufferList, Mesh mesh, Material[] materials, MaterialPropertyBlock mergedMaterialPropertyBlock)
        {
            ...
            for (int j = 0; j <= mesh.subMeshCount - 1; j++)
            {
                ...
                Graphics.DrawMeshInstancedIndirect(mesh, j, materials[j], cellBounds,
                    ...
                    targetCamera,LightProbeUsage.Off);
#else
                Graphics.DrawMeshInstancedIndirect(mesh, j, materials[j], cellBounds,
                    argBufferList[j], 0, mergedMaterialPropertyBlock, shadowMode, RecieveShadows, vegetationLayer,
                    targetCamera);
