Shader "Custom/HedgedTest" {
    Properties {
		...
		_LocalWorld ("Local or World", Range(0,1)) = 0
    }
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	float2 world = floor(IN.worldPos * _Density);
	float2 local = floor(IN.uv_MainTex * _Density)/2;
	float2 c = lerp(local,world,_LocalWorld);
	float checker = frac(c.x/2 + c.y/2) * 2;
	...
	float3 primaryTex = (step(sphereNoise - _DisLineWidth,_DisAmount) * col.rgb);
